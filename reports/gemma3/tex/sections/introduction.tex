...
I would say the accurate implementation matters too. For example in RMSNorm, Llama does x.to(float16) * w whilst Gemma is (x.to(float) * w).to(float16). The former rounds the results, while the later performs the calculation in 32 bit precision first and then rounds it. This slight difference in implementation makes model be better in higher context lenghts.
