...
def load_all_tensors(model_dir, device: str):
    ...
    print()
    print(
        f"{CYAN}Total model size:{RESET}   {GREEN}{format_size(stats['total'])}{RESET}"
    )
    print(
        f"{CYAN}Embedding layer:{RESET}    {GREEN}{format_size(stats['embed'])}{RESET}"
    )
    print(
        f"{CYAN}Final norm:{RESET}         {GREEN}{format_size(stats['final_norm'])}{RESET}"
    )
    print(
        f"{CYAN}Total MLP:{RESET}          {GREEN}{format_size(stats['mlp_total'])}{RESET}"
    )
    print(
        f"{CYAN}Total Attention:{RESET}    {GREEN}{format_size(stats['attn_total'])}{RESET}"
    )
    print(
        f"{CYAN}Total LayerNorms:{RESET}   {GREEN}{format_size(stats['layernorm_total'])}{RESET}"
    )
    print()
